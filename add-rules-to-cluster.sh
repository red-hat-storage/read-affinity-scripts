...
crush_text_file=${crush_compiled_file/compiled/text}
...
function check_and_append_rule() {
	...
	local rname=$(awk < "$1" ' /^rule\s/ {print $2}')
...
if [[ $processed_rules == 0 ]]; then
	...
	if [[ $warnings -gt 0 ]]; then
		while true; do
			read -r -n 1 -p " Some rules were not applied to the cluster. Continue?[Y/n]" resp
