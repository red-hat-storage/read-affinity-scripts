...
## 
# Following variables are used by scripts that source this file
##
failure_domain_type=""
...
$CEPH -s >/dev/null 2>&1
if [[ $? -ne 0 ]]; then
	echo_error "Could not connect to the ceph cluster (command $CEPH -s failed)"
	exit 1
fi
...
function find_failure_domains()
{
    ...
    while true; do
        ...
        local -i n_children=$(echo "$node_info" | jq ".children | length")
