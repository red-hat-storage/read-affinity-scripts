...
function create_3azs_rule() {
    ...
    sed < $base_dir/$template_dir/$template_3azs -e "s/<<AZ1>>/$az1/;s/<<AZ2>>/$az2/;s/<<AZ3>>/$az3/;s/<<ID>>/$id/;s/<<SFX>>/$rule_suffix/;s/<<TYP>>/$choseleaf_type/"
...
function create_2azs_rule() {
	...
	assert " -n $az1" $LINENO silent
	assert " -n $az2" $LINENO silent
    sed < $base_dir/$template_dir/$template_2azs -e "s/<<AZ1>>/$az1/;s/<<AZ2>>/$az2/;s/<<ID>>/$id/;s/<<SFX>>/$rule_suffix/;s/<<TYP>>/$choseleaf_type/"
...
function check_file() {
	...
	local file=$1
	local rule_name=$(awk < "$file" ' /^rule / { print $2 }')
	if [[ -z $rule_name ]]; then
